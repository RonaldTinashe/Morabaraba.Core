...
let occupy (junction: Junction) (shade: Shade) occupations =
    if Map.containsKey junction occupations then
        Error UnexpectedOccupation
    else
        Map.add junction shade occupations |> Ok
...
let getMills history shade =
    match history with
    | [] -> []
    | { Occupations = occupations } :: _ -> List.filter (isMill occupations shade) lines

let getDefenceMills history =
    match history with
    | [] -> []
    | [ _ ] -> []
    | _ :: ({ Player = defender } :: _ as defenceHistory) -> getMills defenceHistory defender.Shade
...
let validateJunction junction =
    if junction > 0 && junction < 25 then
        ...
        invalidArg "junction"
        <| sprintf "Value passed is %i" junction

let canShoot target history =
    (getShootingMills history |> List.isEmpty
     || getDefenceMills history
        |> List.exists (List.contains target)
